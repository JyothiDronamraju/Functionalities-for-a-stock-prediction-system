...
def predict_stock():
    if request.method == 'POST':
        ...
        shap_values = explainer.shap_values(X[:100])  # Using only a subset of data for performance
        ...
        shap.dependence_plot(0, shap_values, X[:100], show=False)
        plt.savefig("static/shap_dependence_plot.png")
        plt.clf()
        
        return render_template('result.html', symbol=symbol, predicted_prices=predicted_prices, from_date=end_date, news=news, metrics=performance_table)
